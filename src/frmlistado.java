...
public class frmlistado extends javax.swing.JFrame {
    ...
    Connection con;
    DefaultTableModel modeloTodos;
    DefaultTableModel modeloBuscado;

    private JTable tablaTodos, tablaBuscado;
    ...
    private void construirUI() {
        ...
        raiz.add(crearEncabezado("LISTADO DE PACIENTES"), BorderLayout.NORTH);
        raiz.add(crearMenu("listado"), BorderLayout.WEST);
        ...
        JPanel panelTodos = crearPanelTabla("TODOS LOS PACIENTES");
        modeloTodos = new DefaultTableModel() {
            public boolean isCellEditable(int r, int c) { return false; }
        };
        modeloTodos.addColumn("Nombre");
        modeloTodos.addColumn("Apellido");
        modeloTodos.addColumn("Fecha");
        modeloTodos.addColumn("Ocupación");
        modeloTodos.addColumn("C.I.");
        tablaTodos = crearTabla(modeloTodos);
        panelTodos.add(new JScrollPane(tablaTodos), BorderLayout.CENTER);
        panelTablas.add(panelTodos);

        JPanel panelBuscado = crearPanelTabla("RESULTADO DE BÚSQUEDA");
    ...
    private JTable crearTabla(DefaultTableModel modelo) {
        ...
        tabla.setDefaultRenderer(Object.class, new DefaultTableCellRenderer() {
            ...
            public Component getTableCellRendererComponent(JTable t, Object val,
                ...
                if (sel) {
                    setBackground(DORADO);
                    setForeground(new Color(20, 20, 20));
                } else {
                    setBackground(row % 2 == 0 ? VERDE_MENU : VERDE_FILA);
    ...
    private void listarTodosPacientes() {
        ...
        try (PreparedStatement ps = con.prepareStatement(sql);
            ...
            JOptionPane.showMessageDialog(null, "Error al listar pacientes\nContacte al administrador",
    ...
    private void buscarPaciente() {
        if (txtCI.getText().trim().isEmpty()) {
            JOptionPane.showMessageDialog(null,
                "Por favor ingrese un número de C.I. para buscar",
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null,
                "El C.I. debe contener solo números, sin letras ni símbolos",
    ...
    private void eliminarPaciente() {
        if (txtCI.getText().trim().isEmpty()) {
            JOptionPane.showMessageDialog(null,
                "Por favor ingrese el C.I. del paciente que desea eliminar",
                "Campo vacío", JOptionPane.WARNING_MESSAGE);
            return;
        }
        int ci;
        ...
        int confirmar = JOptionPane.showConfirmDialog(null,
            "¿Está seguro que desea eliminar al paciente con C.I. " + ci + "?\n" +
            "Se eliminarán también sus tratamientos y asistencias.\n" +
        ...
        try (PreparedStatement ps = con.prepareStatement(sqlCheck)) {
            ...
            JOptionPane.showMessageDialog(null,
                "Error al verificar el paciente\nContacte al administrador",
                "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        // Eliminar en cascada
        String[] sqls = {
            "DELETE FROM asistencias WHERE ci_paciente = ?",
            "DELETE FROM paciente_tratamiento WHERE ci_paciente = ?",
            "DELETE FROM paciente WHERE ci = ?"
        };
        try {
            for (String sql : sqls) {
                try (PreparedStatement ps = con.prepareStatement(sql)) {
    ...
    private JPanel crearMenu(String activo) {
        ...
        // Un solo listener por botón, solo si no está activo
